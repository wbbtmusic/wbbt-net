...
const junkPhrases = [
    ...
    'Crowd enjoying', 'Abstract', 'AI robotic hands', '0 comments'
];
...
const isJunk = (line) => {
    ...
    if (l.includes('**') && l.length < 20) return true; // Short bold lines? Maybe.
    return false;
};
...
for (let i = 1; i < chunks.length; i++) {
    ...
    while (contentStart < lines.length) {
        ...
        if (lines[contentStart].startsWith('Introduction')) { contentStart++; continue; } // Skip "Introduction" header if at top
        if (!lines[contentStart]) { contentStart++; continue; }
        break;
    }

    // 2. FIND END & NEXT TITLE
    // Strategy: Determine Next Title first (at bottom).
    // Then scan upwards from there. 
    // Any junk line pushes 'contentEnd' up.

    let endScanIndex = lines.length;
    ...
    // Now determine Content End (scanning up from Next Title)
    let contentEnd = endScanIndex;

    for (let j = endScanIndex - 1; j >= contentStart; j--) {
        const line = lines[j];
        ...
        if (isJunk(line)) {
            ...
            // Non-junk. 
            // If we are deep in the footer (near next title), short lines might be recommended titles.
            // Heuristic: If we are 'below' some junk (meaning we hit junk previously scanning up? No, scanning DOWN?)
            // We are scanning UP.
            // If we find a junk line at J, 'contentEnd' becomes J.
            // This means everything below J is discarded.
            // This handles the inter-leaved junk perfectly.
        }
    }
